...
/* 打印在线列表 */
void print_online_list(const char *list_str)
{
    char buf[4096];
    char *token;
...
void *recv_servermsg(void *arg)
{
    ...
    while(1)
    {
        ...
        /* 在线列表：格式 list@ip:port@ip:port@ */
        if(strncmp(rbuf, "list@", 5) == 0)
        {
            print_online_list(rbuf + 5);
            continue;
        }
...
int main()
{
    ...
    /* 告诉服务器：这是正式在线聊天连接 */
    if(send_str_to_server("online") < 0)
    {
        printf("发送 online 失败\n");
        close(tcpsock);
        return -1;
    ...
    while(1)
    {
        ...
        switch(n)
        {
            case 1:
            {
                if(send_str_to_server("getlist") < 0)
                    printf("获取在线列表失败!\n");
                break;
            }
